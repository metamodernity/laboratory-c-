...

{
    static void Main()
    {
        ...
        a1 = (((2 * Math.Sin(Math.Exp(Math.Log10(a * t * t)))) + Math.Exp(Math.Log10(b)) / 2) * ((Math.Cos(Math.Exp(Math.Log10(a * t * t)))) / 2 - Math.Exp(Math.Log10(b))));
        a2 = 9 + ((lncosx * lncosx) + (dter * dter) + (6 * lncosx) + (6 * dter) + ((2 * lncosx) * dter));
        ...
        Console.WriteLine("ИТОГО: 325 ЕДИНИЦ");
        Console.WriteLine("ИТОГО: 227 ЕДИНИЦ");
    }
}
/* Считаем единицы:
